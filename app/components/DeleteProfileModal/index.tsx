...
import { profileId } from '@/app/types';
type props=
{
    id:string;
...
function DeleteProfileModal({id, handleProfileDelete, profileId,setShowDeleteModal, username}:props) {
  return (
    ...
    <button onClick={()=> handleProfileDelete(profileId,id)} className='text-[#e62020]'>ok</button>
    </div>
    </div>
    </div>
  )
